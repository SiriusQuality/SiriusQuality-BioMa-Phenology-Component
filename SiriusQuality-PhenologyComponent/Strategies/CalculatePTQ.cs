...
namespace SiriusQualityPhenology.Strategies
{
	...
	/// <summary>
	///Class CalculatePTQ
    /// CalculatePhtoThermal Quotien  Index on the TTWindowForPTQ thermal time periode
    /// </summary>
	public class CalculatePTQ : IStrategySiriusQualityPhenology
	{
	...
	#region Implementation of IAnnotatable
			...
			{
				get { return "CalculatePhtoThermal Quotien  Index on the TTWindowForPTQ thermal time periode"; }
			}
    ...
    #region Instances of the parameters
			...
			{ 
				get {
						...
						else throw new Exception("Parameter 'TTWindowForPTQ' not found (or found null) in strategy 'CalculatePTQ'");
				 } set {
							VarInfo vi = _modellingOptionsManager.GetParameterByName("TTWindowForPTQ");
							if (vi != null)  vi.CurrentValue=value;
						else throw new Exception("Parameter 'TTWindowForPTQ' not found in strategy 'CalculatePTQ'");
				}
			}
			public Double Kl
			{ 
				get {
						...
						if (vi != null && vi.CurrentValue!=null) return (Double)vi.CurrentValue ;
						else throw new Exception("Parameter 'Kl' not found (or found null) in strategy 'CalculatePTQ'");
				 } set {
							VarInfo vi = _modellingOptionsManager.GetParameterByName("Kl");
							if (vi != null)  vi.CurrentValue=value;
						else throw new Exception("Parameter 'Kl' not found in strategy 'CalculatePTQ'");
				}
			}
	...
	#region pre/post conditions management		
			...
			public string TestPostConditions(SiriusQualityPhenology.PhenologyState phenologystate,SiriusQualityPhenology.PhenologyState phenologystate1, string callID)
			{
				try
				{
					...
					SiriusQualityPhenology.PhenologyStateVarInfo.ListPARTTWindowForPTQ.CurrentValue=phenologystate.ListPARTTWindowForPTQ;
					...
					RangeBasedCondition r6 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.ListPARTTWindowForPTQ);
					if(r6.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.ListPARTTWindowForPTQ.ValueType)){prc.AddCondition(r6);}
					...
					RangeBasedCondition r8 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.PTQ);
					if(r8.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.PTQ.ValueType)){prc.AddCondition(r8);}
			...
			public string TestPreConditions(SiriusQualityPhenology.PhenologyState phenologystate,SiriusQualityPhenology.PhenologyState phenologystate1, string callID)
			{
				try
				{
					...
					SiriusQualityPhenology.PhenologyStateVarInfo.PAR.CurrentValue=phenologystate.PAR;
					...
					RangeBasedCondition r1 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.ListTTShootTTWindowForPTQ);
					if(r1.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.ListTTShootTTWindowForPTQ.ValueType)){prc.AddCondition(r1);}
					RangeBasedCondition r2 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.ListPARTTWindowForPTQ);
					if(r2.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.ListPARTTWindowForPTQ.ValueType)){prc.AddCondition(r2);}
					RangeBasedCondition r3 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.ListGAITTWindowForPTQ);
					if(r3.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.ListGAITTWindowForPTQ.ValueType)){prc.AddCondition(r3);}
					RangeBasedCondition r4 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.PAR);
					if(r4.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.PAR.ValueType)){prc.AddCondition(r4);}
					RangeBasedCondition r5 = new RangeBasedCondition(SiriusQualityPhenology.PhenologyStateVarInfo.DeltaTT);
					if(r5.ApplicableVarInfoValueTypes.Contains( SiriusQualityPhenology.PhenologyStateVarInfo.DeltaTT.ValueType)){prc.AddCondition(r5);}
	...
	#region Model
			...
			{				
                ...
                double parInt = 0.0;
                for (int i = 0; i < phenologystate.ListTTShootTTWindowForPTQ.Count; i++) parInt += phenologystate.ListPARTTWindowForPTQ[i] * (1 - Math.Exp(-Kl * phenologystate.ListGAITTWindowForPTQ[i]));
